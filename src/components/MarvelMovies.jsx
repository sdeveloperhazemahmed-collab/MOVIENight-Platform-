...
export default function MarvelMovies({ movies, setMovies }) {
  const [page, setPage] = useState(1);
  const navigate = useNavigate();

  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/discover/movie?api_key=${import.meta.env.VITE_TMDB_KEY}&with_companies=420&page=${page}`
    )
      .then(res => res.json())
    ...
    <>
      <div className="flex gap-4 w-[100%] h-[451px] m-10">
        <div className="flex flex-col justify-start items-center w-[30%]">
          <img className="w-[250px]" src="marvel.png" />
          <div className="flex flex-col justify-center items-center gap-6">
            <h1 className="text-[var(--light-color)] text-[40px]  text-center">Journey into the Marvel Universe</h1>
